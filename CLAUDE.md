...
- **073**: `productos_estado_por_existencias` — `productos.estado` deja de ser un campo manual y pasa a **derivarse de `cantidad_actual`**. Trigger `BEFORE INSERT OR UPDATE` (`fn_productos_sync_estado_stock()`): `cantidad_actual <= 0` (o NULL) → `'Sin existencias'` **siempre**; `> 0` → `'OK'` **sólo si** venía NULL o `'Sin existencias'`, de modo que `Vencido`/`Perdido`/`Próximo a vencer` (estados de *calidad*, no de disponibilidad) sobreviven mientras haya saldo. Es un trigger y no código de front porque `cantidad_actual` se escribe desde **seis** rutas (`NuevoMovimientoModal`, `NewPurchase`, `PurchaseHistory` al borrar compra, `CierreAplicacion`, `ProductForm` y el importador CSV — este último en la edge function, duplicado en dos árboles). Backfill idempotente incluido (antes: 334 `'OK'` de los cuales **131 con saldo 0**, y 5 `'Sin existencias'` puestos a mano). **Consecuencia que hay que respetar**: cualquier `.eq('estado','OK')` sobre `productos` es desde ahora un filtro de stock encubierto — deseable en la verificación física, **prohibido** en la planeación de aplicaciones (`PasoMezcla` usa `.in('estado', ['OK','Sin existencias'])`, porque ahí se planea antes de comprar y existe el paso de Lista de Compras). Guard estático en `src/__tests__/productosEstadoExistencias.test.ts`.

### Inventory — verification scope

`NuevaVerificacion.tsx` arma la planilla sobre los insumos **con existencias** (`cantidad_actual > 0`), no sobre todo el catálogo activo: contar físicamente un saldo que ya se sabe en cero sólo infla la lista. Casilla opt-in para incluir los agotados cuando se quiera un conteo exhaustivo (buscar saldos que el sistema perdió). Lo que se ve en la tabla es exactamente lo que se inserta en `verificaciones_detalle`, y `ConteoFisico` trabaja sobre ese snapshot — así que el alcance se decide una sola vez, al crear la verificación.
