...
- **047**: `drop_compra_a_gasto_trigger` — drops `trigger_compra_a_gasto` / `crear_gasto_pendiente_de_compra()` (added in 038). Registering a `compra` no longer auto-creates a `Pendiente` `fin_gastos` row — that was causing double-counted expenses when accounting also created the real expense manually. Purchases and expenses are now fully decoupled; `fin_gastos.compra_id` and `fn_cleanup_compra_dependencies` remain for legacy rows.
