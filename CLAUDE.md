...
> |---|---|---|
> | **105** `clima_sync_secreto_compartido` | `/clima/sync` y `/clima/backfill` siguen **abiertos anónimamente desde internet** (el cron postea con solo `Content-Type` y recibe 200 con escritura efectiva). | **Orden obligatorio**: secreto en Vault → `CLIMA_SYNC_SECRET` en la edge function → migración → deploy. Invertirlo deja el cron en 401 cada 5 minutos y abre hueco en `clima_lecturas`. Los dos primeros pasos no se hacen desde una migración. |
...
- **105**: `clima_sync_secreto_compartido` — **NO APLICADA. La exposición que cierra está viva.** Parte de clima del hallazgo ESCO-1 (P1): la edge function corre con `verify_jwt=false` y las rutas `/clima/sync` y `/clima/backfill` no leían ningún encabezado. El job `clima-sync-wu` (jobid 1, migración 030) postea con **solo `Content-Type`** — sin apikey y sin Authorization — y recibe 200 con escritura efectiva, o sea que la exposición es **anónima desde internet**, no "cualquiera con el anon key". No se arregla activando `verify_jwt`: el webhook de Telegram y los pg_cron de 060/102 dependen de que siga en false; la puerta va dentro del handler. Esta migración es **la mitad** del arreglo (reprograma el job para que mande `x-clima-sync-secret`); la otra mitad es el gate `verificarAccesoClima` en `clima.tsx`. **Verificado 2026-08-21 que NO corrió**: el `command` de `clima-sync-wu` no menciona el header. **El orden de aplicación es obligatorio y no es opcional**: (1) crear el secreto en Vault, (2) configurar `CLIMA_SYNC_SECRET` en los secretos de la edge function, (3) aplicar la migración, (4) desplegar la función. Los pasos 1–3 son inofensivos con el código de hoy; recién el 4 empieza a exigirlo. **Al revés (deploy antes que la migración) el cron responde 401 cada 5 minutos** y deja hueco en `clima_lecturas` — falla cerrado, que es lo correcto, pero se pierden lecturas.
