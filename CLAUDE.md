...
| `/hato-lechero/produccion`         | ComingSoon (En Desarrollo, S5)| Hato Lechero   |
...
| `/hato-lechero/alertas`            | ComingSoon (En Desarrollo, S6)| Hato Lechero   |
...
Sequential SQL migrations live in `src/sql/migrations/` (001–064). See `src/sql/migrations/README_MIGRATION.md` for instructions on running them.
...
- **The S4 herd view must show three categories: terneras, hato (milking), horro (dry cows due to calve)** — owner requirement from the same round.
...
`Load` still has never been executed — that is the mandatory human checkpoint with Martha, independent of this endpoint (B0 only ever produces a preview; `Load` writes the *historical* backfill once, separately).

**S4 — Frontend: núcleo del módulo (2026-07-22).** Replaced the `ComingSoon` placeholders at `/hato-lechero`, `/hato-lechero/hato(/:id)` and `/hato-lechero/chequeos` with real views. `/hato-lechero/produccion` (S5) and `/hato-lechero/alertas` (S6) stay `ComingSoon`.
...
- `AnimalesList.tsx` (Hato) — the three categories the owner required (`terneras` / `hato` en ordeño / `horro` secas) as tabs, powered by `utils/hatoCategorias.ts::clasificarCategoriaHato`, a **frontend-only classification with a documented assumption**: it composes on top of the already-engine-derived `EstadoReproductivo` rather than adding a new constant to `calculosHato.ts` (which would require touching the parity-protected server copies for a pure UI/product grouping). The exact hato/horro boundary ("horro" = `estado === 'seca'`, not `proxima_a_secar`) is not spelled out anywhere beyond the owner's one-line requirement — confirm with the owner if this reads wrong in practice.
...
- **Known API contract gap, blocking "approve and write" (flagged for CTO arbitration, not worked around):** the diff review UI is fully built and renders the complete B0 response, but the "Aprobar" action **does not write to the database** — it is intentionally left unwired. `diffChequeos.filas[]` only carries the classification and the *changed* fields (`{ campo, anterior, nuevo }`); it does not include the full raw+normalized row (`FilaChequeoNormalizada` — `pl_raw`, `sx_raw`, `fecha_servicio_raw`, etc., nor the parsed `ResultadoSX` needed for `descomponerSX`) that `hato_chequeo_vacas` (raw + normalized columns) and `hato_eventos` require. Writing with only the partial diff data would either fabricate the raw layer or leave it null for `sin_cambio` rows — both violate the module's "capa cruda conserva la planilla verbatim" rule. Closing this requires a backend decision (out of frontend scope): either `hato-chequeo-preview.ts` returns the full `salida.chequeos: FilaChequeoNormalizada[]` (plus `terneras`/`subtablas` already returned), or a second `/hato/chequeo/commit` endpoint accepts an approved subset and performs the write server-side. See the S4 session report for the exact shape needed.
...
- **Hato Lechero chequeo upload preview (B0/V10)** — `POST /make-server-1ccce916/hato/chequeo/preview` (`hato-chequeo-preview.ts`). Parses an uploaded chequeo `.xlsx` and returns a diff for approval; never commits. See "Hato Lechero Module" above for the full contract.
