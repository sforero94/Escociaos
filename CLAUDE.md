...
Sequential SQL migrations live in `src/sql/migrations/` (001–119). See `src/sql/migrations/README_MIGRATION.md` for instructions on running them.
...
> **Queda UNA migración escrita, fusionada y SIN APLICAR: la 109.** 103 y 104 se aplicaron el 2026-08-21 y **105 el 2026-08-24**, con lo que se cerró la cola histórica de pendientes — pero la 109 abrió una nueva, y es de otra naturaleza: **no está sin aplicar porque nadie la haya corrido, sino porque no se puede correr por SQL.** Ver su entrada abajo. **No la apliques con `apply_migration`: aborta con `42501`.**
>
...
- **109**: `storage_reportes_delete_gerencia` — **fusionada y SIN APLICAR, y no por olvido: el carril de migraciones no puede correrla.** Acota el DELETE del bucket `reportes-semanales` a Gerencia, alineándolo con los otros cuatro buckets que sí llevan políticas (`facturas` → Administrador+Gerencia; los tres del hato → Gerencia, patrón 072). Hoy la política `Authenticated users can delete reports` (migración 019, en el ledger como `021`) tiene el predicado desnudo `bucket_id = 'reportes-semanales'` y `TO authenticated`, sin comprobación de rol: **cualquier autenticado puede borrar los 49 informes archivados**, que son el registro operativo de la finca para GlobalGAP. No es explotable desde fuera — el bucket es privado y `anon` no tiene política — y el padrón son 8 cuentas, todas Gerencia o Administrador; se vuelve real el día que exista una cuenta **Verificador**.
- **113**: `hato_alertas_tick_runs` — **SIN APLICAR** (entregada como parte del hallazgo #4 del PO, 2026-08-24 — "las alertas del hato están calladas y nadie puede decir por qué"). Tabla append-only, una fila por corrida de `POST .../hato/alertas/tick`: el desglose de cobertura de `resumirCoberturaAlertas` (`hatoAlertas.ts` — cuántos animales/pasos se evaluaron y, por cada uno de los 5 tipos de alerta, cuántas se generaron y por qué NO se generaron las demás: `no_activa` · `sin_ciclo_reproductivo` · `evento_no_clasificado` · `sin_servicio_ancla` · `sin_chequeo` · `fecha_futura` · `bajo_umbral` · `ya_generada` · `silenciada`) más los conteos de las fases (b)/(c)/(d) que el handler ya calculaba pero que hasta hoy solo vivían en el cuerpo JSON de una respuesta HTTP que un cron no persiste (ver la lección de la 105 sobre `net._http_response`). Patrón RLS de `acciones_corridas` (101): SELECT `authenticated`, solo `service_role` escribe. Números 110–112 (`delete_globalgap_por_rol`, `cerrar_logs_auditoria`, `productos_updated_by`) pertenecen a ramas paralelas sin fusionar todavía a la fecha de esta entrada — 113 es el siguiente slot libre visto desde esta rama, verificar contra el árbol vivo antes de reutilizar el número si esas se integran primero.
