...
- **067**: `hato_registrar_salida` — `SECURITY INVOKER` plpgsql RPC `fn_hato_registrar_salida(p_animal_id, p_tipo venta|muerte, p_fecha, p_nota?)` backing the "Marcar vendida / muerta" quick action (`HojaDeVida`). One call = one transaction: INSERT `hato_eventos` (tipo venta/muerte, `fuente='web'`, `created_by=auth.uid()`) + UPDATE `hato_animales.estado`/`fecha_estado`. Closes the two-non-atomic-writes gap in `useEventoRapidoHato.ts::marcarSalida` — a failed second write used to leave the animal logged-as-sold but still `estado='activa'` in `v_hato_estado_actual` (that view passes `estado` through directly, unlike `num_partos` which it counts from events). INVOKER, **not** DEFINER (contrast 065): it's called straight from the browser with the user's JWT, so the 044-pattern write RLS on both tables keeps gating it to Administrador/Gerencia with no manual role check. `parto` stays a single insert (already atomic). EXECUTE revoked from PUBLIC+anon, granted only to `authenticated` (the RLS does the role gate). Applied to production 2026-07-24 via the authenticated Supabase MCP (verified: `prosecdef=false`/INVOKER; EXECUTE held only by `authenticated`/service_role/owner).
...
**Known follow-ups (flagged, not fixed):** (1) `dedupe.ts` (Extract) doesn't catch duplicate sheets **across different source files** (see the cross-file case above) — worth a standalone investigation. (2) Nothing populates `hato_chequeo_vacas.meses_prenez` (neither `Load` nor the commit path). (3) `src/types/database.ts` is stale (predates 044) — regenerating it would remove the `as any` casts in the hato/ganado hooks. (4) `scripts/import-hato/verify.ts` is referenced by `load.ts`'s final log line but was never written. (5) 102 animals loaded as `estado='activa'` vs. an estimated ~45 in the real current herd — the D5 365-day presumed-closure heuristic likely didn't catch every real departure; needs a review pass with Martha, corrected via the edit UI, never via re-Load.

**S8 — Figma UI alignment (2026-07-24).** Aligned the whole Hato Lechero UI to the Figma mockups file *"Escocia OS — Módulo Hato Lechero (Mockups)"* (Hato-only, **not** a whole-app prototype — an audit-confirmed scope correction). New shared primitives under `components/`: `HatoPageHeader`, `HatoKpiCard`, `HatoReproCard`, `AnimalLabel`. New pure/tested helpers: `utils/hatoAlertas.ts` (**single source** of the 4 alert signals — secado/parto/rechequeo/servir — consumed by BOTH the Dashboard "Tablero de alertas" and the new Alertas view so they can never diverge), `utils/ordenarAnimalesHato.ts`, `utils/graficoLitrosQuincenal.ts`.
...
- **Hoja de Vida**: PL curve (`CurvaProduccionLeche`, from the per-chequeo `pl` series already in `detalle.chequeos` — labeled "Por chequeo", **not** weekly, since `hato_pesajes_leche` is still empty), Tratamientos card (`useHatoTratamientos`, new query of `hato_tratamientos`/`_pasos`/`_protocolos`), role-gated quick actions Registrar parto / Marcar vendida-muerta (`useEventoRapidoHato`).
...
- **Alertas**: `ComingSoon` → live `AlertasView`.
...
**Alertas is DERIVED only — flagged deviation.** Figma's "Cola de alertas" is the S6 Telegram closed-loop (Enviada/Confirmada/Escalada/Destinatario/Intentos/Respuesta); that backend (tick endpoint + cron, migration 060) is **still unbuilt**, so the view shows only the honestly-fillable columns and carries a visible banner saying delivery tracking arrives with S6. Building that closed-loop is the remaining S6 backend epic.
