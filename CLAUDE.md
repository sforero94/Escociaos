...
> **Tres migraciones escritas y verificadas están SIN APLICAR, y el defecto de cada una sigue vivo en producción** (comprobado contra el catálogo 2026-08-21 — no contra el ledger). Están detalladas abajo; el resumen para no tener que buscarlas:
>
> | | Qué sigue pasando hoy | Al aplicarla |
> |---|---|---|
> | **103** `clima_cobertura_parcial` | Un día capturado a medias se guarda como **0,00 mm con sello `ok`**. Pasó el 2026-08-19 (167 de 288 lecturas, corte de luz). Se repite con cada corte que empiece o termine a media jornada. | Sin dependencias. Se puede aplicar sola. |
> | **104** `rls_verificaciones_inventario` | `verificaciones_inventario` y `verificaciones_detalle` siguen **escribibles con la llave `anon` del bundle** (6 policies `TO public` con predicado `true`). Corrupción silenciosa, no destrucción — no se propaga al inventario real. | Sin dependencias, pero revisá los call sites: la verificación en curso del 2026-07-30 debe seguir siendo terminable. |
> | **105** `clima_sync_secreto_compartido` | `/clima/sync` y `/clima/backfill` siguen **abiertos anónimamente desde internet** (el cron postea con solo `Content-Type` y recibe 200 con escritura). | **Orden obligatorio**: secreto en Vault → `CLIMA_SYNC_SECRET` en la edge function → migración → deploy. Invertirlo deja el cron en 401 cada 5 minutos. |
