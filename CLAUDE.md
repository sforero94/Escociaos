...
- **Climate**: `clima_lecturas` (rolling 24h window of 5-min Ecowitt readings — pruned daily by cron; `lluvia_diaria_actualizada_en` since migration 067 carries Ecowitt's own freshness timestamp for the cumulative daily rain counter), `clima_resumen_diario` (pre-aggregated daily summaries: min/max/avg temp, humidity, wind, rainfall, radiation, UV — one row per day, scales indefinitely; `lluvia_confianza` since migration 067 — see that entry for the frozen-counter bug it fixes)
- **Audit**: `audit_log`
