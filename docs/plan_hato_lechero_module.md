...
- **Cierre parcial (2026-07-22)**: entregado el pipeline **Extract → Normalize → Resolve**, corriendo end-to-end sobre los 7 `.xlsx` reales, más `Load` y `Verify` escritos (Load **no ejecutado**: espera el checkpoint con Martha). Lógica pura en `src/utils/importHato/` (typecheck + lint + Vitest, según el ajuste de `docs/hato/s3-contrato-pipeline.md`); runners de I/O en `scripts/import-hato/`. Suite **922/922**, typecheck limpio, lint 0 errores. Salida real: **35 chequeos** (45 hojas físicas − 10 duplicadas), **1.716 filas** normalizadas, **191 TERNERAS**, **167 animales** en `animales.csv`, y `resolution-report.md` listo para revisar.
  - **Desempates de chapeta**: decisión del dueño (2026-07-22) de numerar **desde 999 hacia atrás** todas las colisiones, en `src/utils/importHato/overridesChapeta.ts`. 11 colisiones concurrentes → 22 animales con número de trabajo; **ningún** animal conserva su número original, así que las 11 chapetas físicas quedan sin dueño en el sistema hasta que entren los definitivos. Son **provisionales**: se editan en ese archivo y se re-corre (Load es idempotente sobre `origen='importacion_historica'`).
  - **Dos defectos que solo aparecieron al correr sobre datos reales** (los 146 tests unitarios no los detectaron) — documentados en [`s3-verificacion-independiente.md`](./hato/s3-verificacion-independiente.md) §3.10: (a) el chequeo de marzo 2019 se cargaba dos veces, porque su título no trae día y el dedupe agrupaba solo por fecha; (b) **nueve animales reales se fusionaban en silencio** al tratar como "renombre" toda colisión que no siguiera vigente en la última lectura — `#43` CUÑA/MONTAÑA coexisten en 25 hojas durante 7 años. Ambos con test de regresión.
  - **Cinco preguntas abiertas cerradas o acotadas por medición** (mismo doc §3.5-§3.8): los 4 bloques de `FLUJO LECHE` sí traen año (orden **descendente** 2026→2023, al revés de lo asumido); las 2 colisiones vigentes son **dos animales reales** (Martha ya las lleva como filas separadas en su planilla de leche); `yaguen`/`fabace` son **nombres de toro**, no razas; `Mv` son **2 animales** retirados de la reproducción; `gem+` no se confirma como gemelos.
  - ~~**Pendiente de S3**: el endpoint de subida de Excel por chequeo (B0/V10) no se construyó~~ → **construido el mismo día**: `POST /make-server-1ccce916/hato/chequeo/preview` (multipart, campo `archivo`, auth Administrador/Gerencia), devuelve un **diff para aprobar y nunca comete**. Lógica pura del diff en `src/utils/importHato/diffChequeo.ts`; `HatoConfig` leído en vivo de `hato_config` vía `hato-config-desde-tabla.ts` (lanza error nombrando claves faltantes, jamás un default silencioso). Los módulos de Extract+Normalize se espejan a los DOS árboles del servidor con `docs/hato/regenerar-copias-importhato.py` (**generados, nunca a mano**; paridad protegida por `importHatoParidadServidor.test.ts`, verificada por mutación). Suite tras el endpoint: **950/950**. ⚠️ Sin runtime de Deno local, el handler no se pudo ejecutar — necesita smoke test tras `npx supabase functions deploy make-server-1ccce916`. `Load` sigue sin ejecutarse: ese es el checkpoint humano, no una tarea de agente.

### Decisiones del dueño — segunda ronda (2026-07-22, sobre `resolution-report.md`)

Santiago respondió las 12 secciones del reporte generado. Las respuestas con efecto en código:

- **§5 Cierres presuntos**: sin tratamiento especial. **Regla nueva: >1 año sin aparecer en un chequeo = ya no está en el hato** (reemplaza "ausente ≥2 chequeos"). Se aplica sola, no se le pregunta a nadie.
- **§6 Homologación de la columna Toro**: Gyr←`gir` · Holstein←`hol/hols/h t/HOLST/toro holst/hins` · Jersey←`jers/jersey/jer/TJ` · Normando←`nor` · toros reales: **Nitro**, **Steem**, **Fabace** (=`FABA`, jersey). **`ok`/`INOOK` no es un toro**: significa "la vaca está ok" (`rech` = rechequeo); la señal es que no hay fecha de servicio asociada. **Dejar la raza como nombre del toro está bien** — no todos los toros tienen nombre (revierte la regla anterior de "una raza jamás aterriza en toroNombre").
- **§7 PADRE en TERNERAS**: `yaguen`/`fabace` son **nombres de toro, no razas; asumir jersey**.
- **§8 Ventas inferidas**: la nota `'chispa. Dacota, indir  vendida'` (CHEQUEO JULIO 2026) basta para marcar **CHISPA, DACOTA e INDIRA como vendidas**, con número de trabajo 800-899 donde haga falta.
- **§9 Sub-tabla de novillas 2024**: intentar resolver contra los chequeos de 2025; si no, ignorar.
- **§10**: `Mv` = "vacas de Martha", **ignorar para el sistema** · `gem+` = **parto gemelar** · fechas en OBS/ESTADO de 2019: ignorar (muy antiguas, sin impacto material) · **litros al camión mide producción del hato (venta); el pesaje por vaca mide productividad individual** — datos distintos, historias distintas, sin atribución por ordeño · **el pesaje se hace los miércoles** (migración 064: `dia_pesaje_semanal`) · Fabace = FABA confirmado · **agregar gyr** al catálogo (migración 064) · la nota de novillas vivirá en observaciones en el sistema nuevo · **El hato tiene tres categorías: terneras, hato (en ordeño) y horro (secas próximas a parir) — el inventario y la vista de S4 deben mostrar esas tres.** *(Superada por la tercera ronda, abajo: son CUATRO.)*

### Decisiones del dueño — tercera ronda (2026-07-22, sobre la regla de categorías implementada en S4/S7)
...
- **Cierre (2026-07-23)**: entregada con el despacho apagado por construcción — el seed de `hato_alertas_config` no trae destinatarios, así que el tick genera y NO envía hasta que se configuren (ese es el interruptor del modo sombra; los checkpoints de confianza siguen vigentes). Motor puro `hatoAlertas.ts` (espejos GENERADOS vía `docs/hato/regenerar-copias-hato-alertas.py`, paridad en `hatoAlertasParidadServidor.test.ts`), endpoint `/hato/alertas/tick` (503 sin secreto, 401 con header inválido; 3 fases con conteos), helper saliente `telegram/enviar.ts` con log a `telegram_mensajes`, callbacks `hato_alerta:{id}:{si|no|otro}` con efectos de dominio, y `AlertasView` (cola + revisión semanal de Martha, V11). Regla del período provisional respetada: el mensaje lidera con el NOMBRE para chapetas provisionales/nulas. Escalamiento anclado a `datos.enviada_en` (solo alertas realmente enviadas escalan — una alerta en sombra jamás escala); expiración >14 días sobre `fecha_programada`. Secretos aprovisionados (Vault `hato_alertas_tick_secret` + edge `HATO_ALERTAS_TICK_SECRET`). Pendientes humanos: configurar destinatarios para encender sombra, alta de Fernando (S5 doc), y decidir si el destinatario de escalamiento pasa de env var a columna en `hato_alertas_config`.
...
- **Cierre (2026-07-23)**: entregada. "Registrar venta" reusa `TransaccionGanadoForm` con 3 props opcionales (sin props = camino ceba byte-idéntico, 4 call sites auditados); al guardar escribe `es_hato`/`hato_animal_id`, crea `hato_eventos` `'venta'` (vía la columna FK real `transaccion_ganado_id` de 053) y marca `estado='vendida'`. "Registrar muerte" NO pasa por finanzas: diálogo propio → evento `'muerte'` + `estado='muerta'`. Un fallo post-guardado cierra el diálogo con toast explícito de qué quedó a medias (dejar el diálogo abierto invitaba a duplicar la transacción financiera). Lógica pura en `hatoSalida.ts` (10 tests).
...
- **Cierre (2026-07-23)**: entregada. `PajillasView` en `/hato-lechero/pajillas` (6º ítem del sidebar agregado en `Layout.tsx`): catálogo de toros (SELECT-or-INSERT sobre `lower(nombre)`, colisión `23505` con mensaje claro), stock por lote desde `v_hato_pajillas_stock` (chip ámbar en ≤0, nunca bloquea), compra + uso append-only (animal opcional, sin efectos colaterales). `AjustesHato` como tab Gerencia-only en Configuración global: edita las 11 claves de `hato_config` con validación pura (`ajustesHatoValidacion.ts`) que hace round-trip por `construirHatoConfigDesdeFilas` en tests — una regresión de forma revienta en Vitest, no en producción; persiste UPDATE-by-`clave`, jamás upsert.
