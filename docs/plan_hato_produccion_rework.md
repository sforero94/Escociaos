...
| R3 | ~~**Tailwind congelado.** `src/index.css` es un build precompilado; una clase ausente no falla, simplemente no hace nada.~~ **Superado el 2026-08-06 por F1 de `docs/plan_tailwind_pipeline.md`.** | ~~Verificar con `grep -cF` antes de usar cualquier utilidad nueva.~~ El compilador corre en cada build: cualquier utilidad válida funciona y no hay nada que comprobar. CSS a mano en `globals.css` solo como excepción y **dentro de `@layer`**. Aplica también al riesgo R-11 y al guard estático de SOW 5, retirado en F1. |
