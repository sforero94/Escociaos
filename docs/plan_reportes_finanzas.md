...
| ~~**Clases de Tailwind congeladas**: `table-fixed`, `tabular-nums`, `border-collapse` y los anchos arbitrarios **no existen** en `index.css` y se ignoran en silencio~~ **Superado el 2026-08-06 por F1 de `docs/plan_tailwind_pipeline.md`: Tailwind compila y esas utilidades funcionan.** | Los anchos, la alineación numérica y la columna congelada se definen como CSS real en `globals.css` (`.tabla-financiera`, `.celda-num`, `.col-etiqueta`) — **se quedan**, porque son estructura de tabla, no un parche por clase ausente. ~~Copiar `PresupuestoTable.tsx` como plantilla propagaría el bug.~~ |
