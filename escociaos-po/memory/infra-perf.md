...
- **Linea de salud**: Vercel 5/5 READY + 1 CANCELED benigno (cuatro merges en 100 s, Vercel cancela el build superado — **no filarlo como build fallido**), alias PROMOTED en HEAD, build 29,0 s · **edge `make-server-1ccce916` v238 del 2026-09-01, SIN DESPLEGAR desde entonces** · 0 errores de edge en 293 peticiones/24 h · consulta de aplicacion mas lenta `fn_clima_rollup_diario()` 219,9 ms 1x/dia · DB **119 MB** (1,5% de 8 GB; 78 MB son maquinaria: `net._http_response` 59 MB, `cron.job_run_details` 19 MB) · **6 pg_cron, 2.051 corridas en 7 dias, 0 fallos** · `clima_lecturas` 3 min de atraso, 288/288 los dias 09-05 y 09-06.
- **Advisors por SQL**: unused_idx 82 (era 77) · sin_pk 14 · 110 tablas. **Ninguno es hallazgo.**
- **LA SONDA HTTP SUSTITUYE A LA DESCARGA DEL BUNDLE.** Ver `_compartida.md`. `/health` 200 + ruta nueva 404 = bundle anterior a esa ruta, sin depender de `updated_at` ni de hashes guardados.
- **El detector de deriva `deteccion-deriva-despliegue.yml` NO tiene ningun paso de aviso.** Escribe en `$GITHUB_STEP_SUMMARY` y sale distinto de cero; el `actor` del cron es `thinksid`, asi que el correo va a `subs@thinksid.co`. **Rojo desde el 09-04 (corridas 12, 13, 14) y nadie se entero en 3 dias.** Un `conclusion: failure` en Actions NO es una notificacion. Filado aparte del #66: aquel es la comparacion, este es la ausencia de aviso.
- **`net._http_response` retiene ~6 horas exactas.** Los ticks de 10:45/10:50/11:00 UTC caben; el de 12:00 no ha corrido cuando el barrido de las 11:00 lo busca. **Consultarla SIN join a `net.http_request_queue`** — la cola se drena y el join borra filas que si existen.
