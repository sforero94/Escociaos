...
- **La cuota de artefactos de Actions es de la CUENTA, la API es por REPOSITORIO.** `GET /repos/{owner}/{repo}/actions/artifacts` no ve lo que acumulen otros repos de `thinksid`. Hoy no importa (`sforero94/Escociaos` tiene 0 corridas de Actions), pero si el umbral se respeta y la cuota sigue llena, la respuesta esta afuera de ese repo. **El techo real del plan no se pudo determinar desde la sesion** — la evidencia (subidas OK a ~2,01 GB el 08-11, bloqueo el 08-12) apunta a ~2 GB, pero RESTORE.md documenta 0,5 GB incluidos; se dejo el umbral de purga en 800 MB, que es el consumo de regimen que el propio diseno declara. Confirmar contra la pagina de billing antes de subirlo. [corrida: 2026-08-13-jueves]
