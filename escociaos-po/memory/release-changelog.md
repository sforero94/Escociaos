...
- **Un hueco en `rondas_avisos` NO siempre es un bundle regresivo.** El verificador lo probo: `resolverDestinatarios` devuelve `["null", "<telegram_id de un suscriptor valido>"]`, longitud 2, asi que la guarda de `length===0` nunca dispara y **el recordatorio de septiembre SI se entrego**. Antes de culpar al despliegue, comprobar si alguna clave quedo reclamada sin envio — y si hay un co-destinatario valido.
