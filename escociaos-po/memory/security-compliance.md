...
- **Baseline**: 110 tablas en `public`, 110 con RLS (100%), **311 politicas**, 13 funciones SECURITY DEFINER (todas con `search_path` fijado). Padron SIN CAMBIOS: 6 Gerencia + 3 Administrador + 1 Verificador = 10 activas, 0 inactivas. `uriel@escocia.com` sigue con `last_sign_in_at IS NULL`. **No aparecio una segunda cuenta no humana: la condicion de reapertura del #59 NO se cumple.**
- **LA 133 ESTA APLICADA Y LA CLASE INSERT/UPDATE always-true ESTA EN CERO** (eran 33 sobre 20 tablas). La de DELETE/ALL devuelve 1, `reportes_semanales TO service_role`, que siempre se descuenta. **No re-filar la clase.**
- **HALLAZGO P1 NUEVO — las DOS primitivas de autorizacion no miran lo mismo.** `es_usuario_gerencia()` lleva `AND usuarios.activo = true`; **`get_user_role()` NO**. Medido y re-derivado por un verificador con otra forma de consulta: **125 politicas llaman a `get_user_role()`, y el desglose por `cmd` da ALL 37 / DELETE 21 / INSERT 33 / UPDATE 23 / SELECT 11 = 114 de ESCRITURA sobre 46 tablas**. 51 llaman a `es_usuario_gerencia()`. **0 politicas RESTRICTIVE en toda la base.** Ni `AuthContext` ni `ProtectedRoute` miran `activo`; `usuarios.tsx` nunca manda `ban_duration`.
- **EL HECHO MAS FUERTE, que el hallador no vio**: `usuarios` tambien esta gateada por `get_user_role()`, asi que **una Gerencia desactivada puede reactivarse ella misma**.
- **REGLA GENERAL que se gana el sitio**: cuando un esquema tiene DOS primitivas de autorizacion, comprobar que las dos miren las MISMAS columnas. La que se olvido de una es la que se usa en el 40% de las politicas.
- **La 109 cerro DELETE y dejo UPDATE e INSERT abiertos** en `reportes-semanales` (51 objetos, **32 ya sobrescritos** legitimamente por el `upsert` de `reporteSemanalService.ts`, lo que hace indistinguible una sobrescritura no autorizada). Filado esta corrida. Conserva su limite duro: `ALTER POLICY` sobre `storage.objects` exige ser DUENO, solo por el panel, y no deja fila en el ledger.
