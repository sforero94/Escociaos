...
import type { FiltrosFinanzas, Negocio, Region, CategoriaIngreso } from '../../../types/finanzas';

interface FiltrosIngresosExtendidos extends FiltrosFinanzas {
  categoria_id?: string;
}

interface FiltrosIngresosProps {
  filtros: FiltrosIngresosExtendidos;
  onFiltrosChange: (filtros: FiltrosIngresosExtendidos) => void;
  onAplicarFiltros: () => void;
}

/**
 * Componente de filtros para la vista de ingresos
 * Extiende FiltrosGlobales con filtro específico de categoría
 */
export function FiltrosIngresos({ filtros, onFiltrosChange, onAplicarFiltros }: FiltrosIngresosProps) {
  ...
  const cargarCatalogos = async () => {
    try {
      ...
      const [negociosResult, regionesResult, categoriasResult] = await Promise.all([
  ...
  const handleFiltroChange = (campo: keyof FiltrosIngresosExtendidos, valor: string | string[] | undefined) => {
    onFiltrosChange({
      ...filtros,
      [campo]: valor
    });
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Select
            value={filtros.categoria_id || 'todos'}
            onValueChange={(value) => handleFiltroChange('categoria_id', value === 'todos' ? '' : value)}
            disabled={loading}
          >
            <SelectTrigger>
      ...
      {/* Segunda fila con botón aplicar */}
