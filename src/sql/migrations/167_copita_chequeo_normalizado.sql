...
--
-- LA CAPA CRUDA NO SE TOCA (contrato de la 080). Consecuencia conocida y
-- deliberada: `fecha_servicio_raw` sigue diciendo '12/08/26', asi que RE-COMITEAR
-- ese chequeo volveria a generar el servicio falso. Eso es un defecto aparte, ya
-- anotado en el CLAUDE.md raiz bajo la 163, y esta migracion no lo cierra.
--
...
-- ---------------------------------------------------------------------------
-- 2. La correccion -- solo columnas NORMALIZADAS, la capa cruda intacta
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- LA CAPA CRUDA NO SE TOCO -- esto es contrato, no decoracion
  IF v.pp_raw IS DISTINCT FROM '13/11/2026'
     OR v.secar_raw IS DISTINCT FROM '13/9/2026'
     OR v.fecha_servicio_raw IS DISTINCT FROM '12/08/26' THEN
    RAISE EXCEPTION '167 post: la capa cruda cambio -- viola el contrato de la 080';
  END IF;
  ...
  SELECT fecha_probable_parto, fecha_secar, meses_prenez, ultimo_servicio_fecha
    INTO v_vista
