// ARCHIVO: utils/calculosHato.ts
// DESCRIPCIÓN: Motor de lógica pura del módulo Hato Lechero (S2 del plan
// docs/plan_hato_lechero_module.md — ver §6, §7.1, §7.3).
//
// Este archivo incorpora dos barridos de evidencia sobre las 8 planillas
// reales: un primer análisis de muestra y un segundo barrido adversarial de
// QA sobre las 45 hojas de chequeo COMPLETAS (fila por fila, no muestreo).
// El segundo barrido refutó cuantitativamente dos supuestos del primero
// (documentados en el sitio donde aplican: `calcularFechaSecar` y
// `calcularMesesPrenez`) y confirmó que ni la columna `TP` ni el número de
// chapeta (`numero`, incluso en el hato ACTUAL) son tan confiables como
// asumía el plan original -- ver `detectarColisionesChapeta` (Bloque 6).
//
// Módulo PURO: no importa el cliente de Supabase ni nada de React/Deno/
// navegador. No usa `Date.now()`/`new Date()` implícito en ningún cálculo de
// negocio -- toda función que depende de "hoy" recibe la fecha de referencia
// como parámetro (mismo patrón que `PriorizacionInput.fechaReferencia` en
// priorizacionMonitoreo.ts). Esto permite que S3 (import histórico) y S6
// (motor de alertas / edge function) reutilicen exactamente esta misma
// lógica sin reimplementarla -- y que sea determinístico en tests.
//
// Regla dura que gobierna TODO este archivo (evidencia real, no supuesto:
// 8 planillas .xlsx del hato 2019-2026 analizadas antes de escribir esto):
// una celda no interpretable JAMÁS descarta la fila. Todo parser devuelve
// `issues: ParseIssue[]` con el valor crudo intacto en vez de lanzar o
// devolver silenciosamente un resultado inventado -- mismo contrato de
// "ausencia de fila = no visto, nunca 0" que ya rige monitoreo/ganado.
// Cuando una heurística de recuperación SÍ logra interpretar un valor roto
// (ej. un año de 3 dígitos), el resultado se entrega junto con un issue que
// documenta qué se asumió -- "ambiguo -> revisión, nunca en silencio".
//
// `HatoConfig` refleja 1:1 las 9 claves sembradas por la migración
// 058_create_hato_config.sql. Ninguna de esas constantes de negocio vive
// hardcodeada en este archivo (plan §7.1/§7.3) -- todo umbral llega como
// parámetro. Construir un `HatoConfig` a partir de las filas de la tabla
// `hato_config` es responsabilidad del caller (hook/edge function), igual
// que `PriorizacionInput` es ensamblado por el caller en
