// ARCHIVO: utils/hatoConfigDesdeTabla.ts
// DESCRIPCIÓN: Traduce las filas crudas de `hato_config` (clave/valor jsonb,
// migraciones 058 + 062) al `HatoConfig` tipado que consume el motor puro
// `calculosHato.ts` -- versión FRONTEND, usada por los hooks de
// `src/components/hato/hooks/` (S4). Deliberadamente NO importa desde
// `src/supabase/functions/server/hato-config-desde-tabla.ts`: ese archivo es
// Deno-side (importa `./calculos-hato.ts` con extensión explícita, y no está
// en la lista de módulos espejados por `regenerar-copias-importhato.py`),
// así que hacer que el bundle del cliente lo importe arrastraría la mitad
// del árbol de la edge function al bundle web sin necesidad. La lógica es
// intencionalmente la MISMA (misma regla dura: ninguna clave faltante cae a
// un default inventado) -- ver ese archivo para el razonamiento completo.
//
// Regla dura (CLAUDE.md, "no business constant lives in the engine"): si
// falta una clave o llega mal tipada, esto FALLA explícito listando TODAS
// las claves con problema de una vez -- nunca un fallback silencioso.

import type { HatoConfig } from '@/utils/calculosHato';

/** Una fila cruda de `hato_config`: `clave text`, `valor jsonb`. */
export interface FilaHatoConfig {
  clave: string;
  valor: unknown;
}

/** Las 10 claves numéricas que `HatoConfig` necesita, sembradas por 058 (9)
 * + 062 (`dias_espera_voluntaria_post_parto`). Espejo de la lista en
 * `hato-config-desde-tabla.ts` (Deno) -- si esa lista cambia, esta debe
  ...
  if (clavesFaltantes.length > 0) {
    throw new Error(
      `hato_config no trae ${clavesFaltantes.length} clave(s) requerida(s) por el motor: ${clavesFaltantes.join(', ')}. ` +
        'Verificar que las migraciones 058 y 062 se aplicaron en este entorno -- el motor nunca usa un default inventado en código.',
    );
  }
